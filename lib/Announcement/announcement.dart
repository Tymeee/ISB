...
class _AnnouncementState extends State<Announcement>{
  ...
  Widget build(BuildContext context) {
    ...
    getData() async{
      var googleAPI;
      var url = Uri.parse('http://54.255.62.155/uploadget.php');
      final post = await http.get(url);
      var data = json.decode(post.body);
      List<AnnouncementModel> announcements = [];
      for(var a in data){
        AnnouncementModel announcement = AnnouncementModel(a["file_name"], a["date"], a["daynumber"], a["day"],);
          ...
          child: Column(
            children: <Widget>[
              ...
              Align(
                ...
                child: Text("Today's Date", style: TextStyle(fontSize: 17),),
              ),
              ...
              Container(
                ...
                child: Center(
                  child: Text(abbrDay + " " + yMMMMD, style: TextStyle(fontStyle: FontStyle.italic, fontWeight: FontWeight.bold, fontSize: 27),),
                )
              ),
              ShareItem().SizedBoxCus(10, 20),
              Align(
                      ...
                      if(snapshot.data == null) {
                        ...
                        return ListView.builder(itemCount: snapshot.data.length,itemBuilder: (context, i) {
                          return Padding(
                            ...
                            child: TextButton(
                              onPressed: () async {
                                final url = 'http://54.255.62.155/uploads/' + snapshot.data[i].file_name;
                                await PDFApi().loadNetwork(url, snapshot.data[i].file_name).then((value) {
                                  setState(() {
                                    localPath = value;
                                  });
                                });
                                  ...
                                  child: Padding(
                                    ...
                                    child: Row(
                                      ...
                                      children: <Widget>[
                                        Align(
                                          alignment: Alignment.centerLeft,
                                          child: Text(snapshot.data[i].day, style: TextStyle(fontWeight: FontWeight.bold, color: Colors.black, fontSize: 17),),
                                        ),
                                        Align(
                                          alignment: Alignment.center,
                                          child: Text(snapshot.data[i].date, style: TextStyle(color: Colors.black, fontSize: 17),),
                                        ),
                                        Align(
                                          alignment: Alignment.center,
                                          child: Text("HS Daily Bulletin", style: TextStyle(color: Colors.black, fontSize: 17),),
                                        ),
                                      ],
                                    ),
                                  ),
